{{-- resources/views/dashboard/levels/level7.blade.php --}}
@extends('layouts.app')
...
<style>
    ...
    .stat-icon-blue-7 { background: rgba(59, 130, 246, 0.12); color: #3b82f6; }
    .stat-icon-green-7 { background: rgba(34, 197, 94, 0.12); color: #22c55e; }
    .stat-icon-orange-7 { background: rgba(245, 158, 11, 0.12); color: #f59e0b; }
    ...
    .quick-action-7 .label {
        font-size: 0.75rem;
        font-weight: 600;
        color: var(--text-primary);
    }
    
    .rank-progress-7 .fill {
        background: linear-gradient(135deg, #ec4899, #db2777);
    }
    .rank-progress-bar-7 {
        width: 100%;
    ...
    .network-level-7:hover { transform: scale(1.02); border-color: #ec4899; }
    .network-level-7 .number { font-size: 1.5rem; font-weight: 700; }
    .network-level-7 .label { font-size: 0.65rem; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    .leader-item-7:hover { background: var(--bg-hover); transform: translateX(4px); border-left-color: #ec4899; }
    .leader-item-7 .rank {
        width: 1.5rem;
        height: 1.5rem;
        display: flex;
    ...
    .leader-item-7 .rank.gold { background: #eab308; color: white; }
    .leader-item-7 .rank.silver { background: #9ca3af; color: white; }
    .leader-item-7 .rank.bronze { background: #d97706; color: white; }
    .leader-item-7 .rank.default { background: var(--bg-secondary); color: var(--text-secondary); }
    .leader-item-7 .badge-leader {
        font-size: 0.6rem;
    ...
    .objective-bar-7 .fill { height: 100%; border-radius: 9999px; transition: width 1.5s cubic-bezier(0.4, 0, 0.2, 1); }
    .objective-bar-7 .fill.pink { background: linear-gradient(135deg, #ec4899, #db2777); }
    .objective-bar-7 .fill.blue { background: linear-gradient(135deg, #3b82f6, #2563eb); }
    .objective-bar-7 .fill.green { background: linear-gradient(135deg, #22c55e, #16a34a); }
    ...
    .activity-item-7:last-child { border-bottom: none; }
    ...
    .condition-item-7.met { border-color: #22c55e; background: rgba(34, 197, 94, 0.05); }
    .condition-item-7.unmet { border-color: #ef4444; background: rgba(239, 68, 68, 0.05); }
    .condition-item-7 .check { font-size: 0.8rem; flex-shrink: 0; font-weight: 700; }
    .condition-item-7 .check.met { color: #22c55e; }
    .condition-item-7 .check.unmet { color: #ef4444; }
    .condition-item-7 .current-value { font-size: 0.6rem; color: var(--text-secondary); white-space: nowrap; }
    
    @media (max-width: 640px) {
        .stat-card-7 { padding: 0.75rem; }
        .quick-action-7 { padding: 0.75rem; }
        .quick-action-7 .icon { width: 1.25rem; height: 1.25rem; }
        .network-level-7 .number { font-size: 1.25rem; }
        .card-7 { padding: 0.875rem; }
        .leader-item-7 { padding: 0.375rem 0.5rem; }
        .condition-item-7 { font-size: 0.6rem; padding: 0.2rem 0.4rem; flex-wrap: wrap; }
        .condition-item-7 .current-value { font-size: 0.5rem; }
    }
</style>
@endpush
...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="welcome-card-7 animate-fadeInUp delay-1">
        <div>
            <h3 class="font-bold text-[var(--text-primary)] text-base sm:text-lg">Saphire Manager atteint</h3>
            <p class="text-sm text-[var(--text-secondary)]">
                Vous etes maintenant au niveau 7 - Saphire Manager. Vous etes un leader accompli.
                Continuez a developper vos leaders pour atteindre le niveau 8 - Diamant Bleu.
            </p>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 animate-fadeInUp delay-2">
        <div class="stat-card-7">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg sm:text-xl md:text-2xl font-bold text-pink-600">{{ $currentRankLevel ?? 1 }}</p>
                </div>
        ...
        <div class="stat-card-7">
            <div class="flex items-center justify-between">
                ...
                <div class="stat-icon stat-icon-blue-7">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="stat-card-7">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg sm:text-xl md:text-2xl font-bold text-green-500">{{ $level1 + $level2 + $level3 + $level4 + ($level5 ?? 0) + ($level6 ?? 0) + ($level7 ?? 0) }}</p>
                </div>
                <div class="stat-icon stat-icon-green-7">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="stat-card-7">
            <div class="flex items-center justify-between">
                ...
                <div class="stat-icon stat-icon-orange-7">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/>
                    </svg>
                </div>
            ...
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <span class="badge badge-success text-[10px] sm:text-xs inline-block mt-0.5">
                        {{ $currentRankName ?? 'Saphire Manager' }}
                    </span>
                </div>
            </div>
        ...
        <div class="card-7">
            ...
            <div class="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                @forelse($topDownlines->take(15) as $index => $downline)
                    <div class="leader-item-7 {{ $index < 3 ? 'highlight' : '' }}">
            ...
            <div class="space-y-3">
                <div>
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-semibold">{{ $totalDownlines }} / 30</span>
                    </div>
                    <div class="objective-bar-7">
                        <div class="fill pink" style="width: {{ min(($totalDownlines / 30) * 100, 100) }}%;"></div>
                    </div>
                </div>
                <div>
                    <div class="flex justify-between text-sm">
                        <span class="text-[var(--text-secondary)]">PV mensuel</span>
                        <span class="font-semibold">{{ number_format($pvPersonnel) }} / 12000</span>
                    </div>
                    <div class="objective-bar-7">
                        <div class="fill blue" style="width: {{ min(($pvPersonnel / 12000) * 100, 100) }}%;"></div>
                    </div>
                </div>
                <div>
                    <div class="flex justify-between text-sm">
    ...
    <!-- Rank Progress -->
    <div class="card-7 animate-fadeInUp delay-5">
        ...
        <div class="rank-progress-bar-7 rank-progress-7">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 animate-fadeInUp delay-6">
        ...
        <div class="card-7 lg:col-span-2">
            ...
            <div class="h-40 sm:h-48 flex items-end gap-1 sm:gap-2">
                ...
                @forelse($monthlyData ?? [] as $data)
                    ...
                    <div class="flex-1 flex flex-col items-center group relative">
                        <div class="graph-bar w-full bg-pink-500/30 hover:bg-pink-500 transition-all duration-300"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 animate-fadeInUp delay-7">
        <a href="{{ route('network.index') }}" class="quick-action-7">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
            </svg>
            <span class="label">Mon reseau</span>
        </a>
        <a href="{{ route('commissions.index') }}" class="quick-action-7">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <span class="label">Commissions</span>
        </a>
        <a href="{{ route('wallet.index') }}" class="quick-action-7">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ route('rank.index') }}" class="quick-action-7">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/>
            </svg>
