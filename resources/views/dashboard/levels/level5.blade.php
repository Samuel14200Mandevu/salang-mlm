...
<style>
    ...
    .quick-action-5 .label {
        font-size: 0.75rem;
        font-weight: 600;
        color: var(--text-primary);
    }
    
    .rank-progress-5 .fill {
        background: linear-gradient(135deg, #eab308, #ca8a04);
    }
    .rank-progress-bar-5 {
        width: 100%;
    ...
    .network-level-5:hover { transform: scale(1.02); border-color: #eab308; }
    .network-level-5 .number { font-size: 1.5rem; font-weight: 700; color: #eab308; }
    .network-level-5 .label { font-size: 0.65rem; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    .leader-item-5:hover { background: var(--bg-hover); transform: translateX(4px); border-color: #eab308; }
    .leader-item-5 .rank {
        width: 1.5rem;
        height: 1.5rem;
        display: flex;
    ...
    .leader-item-5 .rank.gold { background: #eab308; color: white; }
    .leader-item-5 .rank.silver { background: #9ca3af; color: white; }
    .leader-item-5 .rank.bronze { background: #d97706; color: white; }
    .leader-item-5 .rank.default { background: var(--bg-secondary); color: var(--text-secondary); }
    .leader-item-5 .badge-leader {
        font-size: 0.6rem;
    ...
    .objective-bar-5 .fill { height: 100%; border-radius: 9999px; transition: width 1.5s cubic-bezier(0.4, 0, 0.2, 1); }
    .objective-bar-5 .fill.gold { background: linear-gradient(135deg, #eab308, #ca8a04); }
    .objective-bar-5 .fill.blue { background: linear-gradient(135deg, #3b82f6, #2563eb); }
    .objective-bar-5 .fill.green { background: linear-gradient(135deg, #22c55e, #16a34a); }
    ...
    .activity-item-5:last-child { border-bottom: none; }
    ...
    .condition-item-5.met { border-color: #22c55e; background: rgba(34, 197, 94, 0.05); }
    .condition-item-5.unmet { border-color: #ef4444; background: rgba(239, 68, 68, 0.05); }
    .condition-item-5 .check { font-size: 0.8rem; flex-shrink: 0; font-weight: 700; }
    .condition-item-5 .check.met { color: #22c55e; }
    .condition-item-5 .check.unmet { color: #ef4444; }
    .condition-item-5 .current-value { font-size: 0.6rem; color: var(--text-secondary); white-space: nowrap; }
    
    @media (max-width: 640px) {
        .stat-card-5 { padding: 0.75rem; }
        .quick-action-5 { padding: 0.75rem; }
        .quick-action-5 .icon { width: 1.25rem; height: 1.25rem; }
        .network-level-5 .number { font-size: 1.25rem; }
        .card-5 { padding: 0.875rem; }
        .leader-item-5 { padding: 0.375rem 0.5rem; }
        .condition-item-5 { font-size: 0.6rem; padding: 0.2rem 0.4rem; flex-wrap: wrap; }
        .condition-item-5 .current-value { font-size: 0.5rem; }
    }
</style>
@endpush
...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="welcome-card-5 animate-fadeInUp delay-1">
        <div>
            <h3 class="font-bold text-[var(--text-primary)] text-base sm:text-lg">Manager Senior atteint</h3>
            <p class="text-sm text-[var(--text-secondary)]">
                Vous etes maintenant au niveau 5 - Manager Senior. Vous etes un leader accompli.
                Continuez a developper vos leaders pour atteindre le niveau 6 - Directeur Envolée.
            </p>
        ...
        <div class="stat-card-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg sm:text-xl md:text-2xl font-bold text-yellow-600">{{ $currentRankLevel ?? 1 }}</p>
                </div>
        ...
        <div class="stat-card-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg sm:text-xl md:text-2xl font-bold text-green-500">{{ $level1 + $level2 + $level3 + $level4 + ($level5 ?? 0) }}</p>
                </div>
        ...
        <div class="card-5">
            ...
            <div class="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                @forelse($topDownlines->take(15) as $index => $downline)
                    <div class="leader-item-5 {{ $index < 3 ? 'highlight' : '' }}">
            ...
            <div class="space-y-3">
                <div>
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-semibold">{{ $totalDownlines }} / 20</span>
                    </div>
                    <div class="objective-bar-5">
                        <div class="fill gold" style="width: {{ min(($totalDownlines / 20) * 100, 100) }}%;"></div>
                    </div>
                </div>
                <div>
                    <div class="flex justify-between text-sm">
                        <span class="text-[var(--text-secondary)]">PV mensuel</span>
                        <span class="font-semibold">{{ number_format($pvPersonnel) }} / 5000</span>
                    </div>
                    <div class="objective-bar-5">
                        <div class="fill blue" style="width: {{ min(($pvPersonnel / 5000) * 100, 100) }}%;"></div>
                    </div>
                </div>
                <div>
                    <div class="flex justify-between text-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-3 sm:gap-4 animate-fadeInUp delay-4">
            ...
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <span class="badge badge-success text-[10px] sm:text-xs inline-block mt-0.5">
                        {{ $currentRankName ?? 'Manager Senior' }}
                    </span>
                </div>
            </div>
    ...
    <!-- Rank Progress -->
    <div class="card-5 animate-fadeInUp delay-5">
        ...
        <div class="rank-progress-bar-5 rank-progress-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 animate-fadeInUp delay-6">
        ...
        <div class="card-5 lg:col-span-2">
            ...
            <div class="h-40 sm:h-48 flex items-end gap-1 sm:gap-2">
                ...
                @forelse($monthlyData ?? [] as $data)
                    ...
                    <div class="flex-1 flex flex-col items-center group relative">
                        <div class="graph-bar w-full bg-yellow-500/30 hover:bg-yellow-500 transition-all duration-300"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 animate-fadeInUp delay-7">
        <a href="{{ route('network.index') }}" class="quick-action-5">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
            </svg>
            <span class="label">Mon reseau</span>
        </a>
        <a href="{{ route('commissions.index') }}" class="quick-action-5">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <span class="label">Commissions</span>
        </a>
        <a href="{{ route('wallet.index') }}" class="quick-action-5">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ route('rank.index') }}" class="quick-action-5">
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/>
            </svg>
