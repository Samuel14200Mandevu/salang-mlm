...
<div class="space-y-6">
    ...
    <div class="animate-fadeInUp">
        ...
        <p class="text-[var(--text-secondary)] mt-1">Choisissez l'abonnement qui correspond à vos objectifs</p>
    </div>
    ...
    <div class="card animate-fadeInUp delay-1 border-l-4 border-primary-500">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <p class="text-sm text-[var(--text-secondary)]">Votre abonnement actuel</p>
            ...
            <div>
                <span class="badge {{ Auth::user()->package_id ? 'badge-success' : 'badge-danger' }} text-sm">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        @forelse($subscriptions as $package)
            @php
                ...
                $icons = ['🚀', '🥈', '🥉', '🥇', '💎'];
                $icon = $icons[$package->id - 1] ?? '📦';
                $isPopular = $package->id == 4;
            @endphp

            <div class="subscription-card card text-center {{ $isCurrent ? 'current' : '' }} animate-fadeInUp delay-{{ $loop->index + 2 }}">
                @if($isPopular)
                    <span class="subscription-badge subscription-badge-popular">⭐ Populaire</span>
                @endif
                
                <span class="sub-icon">{{ $icon }}</span>
                <h3 class="text-xl font-bold text-[var(--text-primary)]">{{ $package->name }}</h3>
                <p class="text-3xl font-bold text-primary-500 mt-2">${{ number_format($package->price, 2) }}</p>
                <p class="text-sm text-[var(--text-secondary)]">{{ $package->pv_value ?? 0 }} PV</p>

                <div class="mt-4 space-y-1 border-t border-[var(--border-color)] pt-4 text-left">
                ...
                <div class="mt-4">
                    @if($isCurrent)
                        <span class="badge badge-success">Abonnement actuel</span>
                    @elseif($isLocked)
                        <span class="badge badge-danger">🔒 Verrouillé</span>
                    @else
                        <div class="space-y-2">
        ...
        @empty
            <div class="col-span-full text-center py-12">
                ...
                <h3 class="text-xl font-semibold text-[var(--text-primary)]">Aucun abonnement disponible</h3>
                <p class="text-[var(--text-secondary)] mt-2">Revenez plus tard pour découvrir nos offres</p>
            </div>
        @endforelse
    </div>
</div>
