...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 animate-fadeInUp">
        <div>
            ...
            <p class="text-sm sm:text-base text-[var(--text-secondary)] mt-0.5 sm:mt-1">
                Gerer les consultations des patients
            </p>
        </div>
        <div class="flex items-center gap-3">
            <span class="px-3 py-1 bg-yellow-500/20 text-yellow-600 rounded-full text-sm font-semibold">
    ...
    <div class="stats-grid grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 animate-fadeInUp delay-1">
        <div class="card-stats border-l-4 border-blue-600">
            ...
            <p class="text-xl sm:text-2xl font-bold text-blue-600">{{ $consultations->total() }}</p>
        </div>
        <div class="card-stats border-l-4 border-yellow-500 animate-fadeInUp delay-2">
            ...
            <p class="text-xl sm:text-2xl font-bold text-yellow-500">{{ $pendingCount ?? 0 }}</p>
        </div>
        <div class="card-stats border-l-4 border-blue-400 animate-fadeInUp delay-3">
            ...
            <p class="text-xl sm:text-2xl font-bold text-blue-400">{{ $consultations->where('status', 'processing')->count() }}</p>
        </div>
        <div class="card-stats border-l-4 border-green-500 animate-fadeInUp delay-4">
            <p class="text-[10px] sm:text-xs text-[var(--text-secondary)] uppercase tracking-wider">Terminees</p>
            <p class="text-xl sm:text-2xl font-bold text-green-500">{{ $consultations->where('status', 'completed')->count() }}</p>
        </div>
    </div>
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="text-xs sm:text-sm">Motif</th>
                ...
                <tbody>
                    @forelse($consultations as $consultation)
                        <tr class="consultation-row @if($consultation->status == 'pending') bg-yellow-500/5 @endif">
                            <td class="font-mono text-xs sm:text-sm text-blue-600">{{ $consultation->id }}</td>
                            <td class="text-sm sm:text-base">{{ $consultation->cashier?->name ?? 'N/A' }}</td>
                            <td class="text-sm sm:text-base">{{ $consultation->nom_complet }}</td>
                            <td class="text-sm sm:text-base max-w-[150px] truncate">{{ $consultation->reason ?? 'N/A' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-6 sm:py-8 text-[var(--text-secondary)] text-sm sm:text-base">
                                ...
                                <p class="text-sm text-[var(--text-tertiary)]">Les fiches apparaitront ici</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
