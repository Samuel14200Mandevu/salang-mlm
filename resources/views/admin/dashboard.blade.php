...
<div class="space-y-6">
    ...
    <div class="animate-fadeInUp">
        <h1 class="text-2xl md:text-3xl font-bold text-[var(--text-primary)]">📊 Dashboard Admin</h1>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <div class="card-stats animate-fadeInUp delay-1 border-l-4 border-primary-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-[var(--text-secondary)] uppercase tracking-wider">Utilisateurs</p>
                    <p class="text-xl md:text-2xl font-bold text-primary-500">{{ $totalUsers ?? 0 }}</p>
                </div>
            ...
            <div class="mt-3 flex justify-between text-xs border-t border-[var(--border-color)] pt-2">
        ...
        <div class="card-stats animate-fadeInUp delay-2 border-l-4 border-green-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl md:text-2xl font-bold text-green-500">${{ number_format($totalCommissions ?? 0, 2) }}</p>
                </div>
            ...
            <div class="flex justify-between text-xs">
                <span class="text-[var(--text-secondary)]">Payées</span>
        ...
        <div class="card-stats animate-fadeInUp delay-3 border-l-4 border-purple-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-[var(--text-secondary)] uppercase tracking-wider">Retraits</p>
                    <p class="text-xl md:text-2xl font-bold text-purple-500">${{ number_format($totalWithdrawn ?? 0, 2) }}</p>
                </div>
            ...
            <div class="flex justify-between text-xs">
                <span class="text-[var(--text-secondary)]">Traités</span>
        ...
        <div class="card-stats animate-fadeInUp delay-4 border-l-4 border-blue-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-[var(--text-secondary)] uppercase tracking-wider">Packages</p>
                    <p class="text-xl md:text-2xl font-bold text-blue-500">{{ $totalPackages ?? 0 }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="card lg:col-span-2 animate-fadeInUp delay-5">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-semibold text-[var(--text-primary)]">📈 Inscriptions mensuelles</h3>
        ...
        <div class="card animate-fadeInRight delay-6">
            <h3 class="font-semibold text-[var(--text-primary)] mb-4">📦 Distribution</h3>
            <div class="space-y-3">
                @foreach($packageDistribution ?? [] as $pkg)
                    ...
                    <div>
                        <div class="flex justify-between text-sm">
                            <span class="text-[var(--text-secondary)]">{{ $pkg->name }}</span>
                            <span class="font-semibold text-primary-500">{{ $pkg->users_count }}</span>
                        </div>
            ...
            <div class="mt-4 p-3 bg-[var(--bg-secondary)] rounded-lg text-center">
                <p class="text-2xl font-bold text-primary-500">{{ $totalUsers ?? 0 }}</p>
                <p class="text-xs text-[var(--text-secondary)]">Total utilisateurs</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="card animate-fadeInLeft delay-7">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-semibold text-[var(--text-primary)]">🆕 Derniers inscrits</h3>
                <span class="badge badge-neutral text-xs">{{ $recentUsers->count() ?? 0 }}</span>
            </div>
                    ...
                    <tbody>
                        @forelse($recentUsers ?? [] as $user)
                            <tr>
                                ...
                                <td class="hidden sm:table-cell text-[var(--text-secondary)] text-sm">{{ $user->email }}</td>
                                <td class="text-right">
                        ...
                        @empty
                            <tr><td colspan="3" class="text-center py-4 text-[var(--text-secondary)]">Aucun utilisateur</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="mt-3 text-right">
                <a href="{{ route('admin.users') }}" class="text-sm text-primary-500 hover:text-primary-600 font-semibold transition">
                    Voir tous →
                </a>
            </div>
        </div>
        ...
        <div class="card animate-fadeInRight delay-8">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-semibold text-[var(--text-primary)]">💰 Dernières commissions</h3>
                <span class="badge badge-neutral text-xs">{{ $recentCommissions->count() ?? 0 }}</span>
            </div>
                ...
                @forelse($recentCommissions ?? [] as $commission)
                    <div class="activity-item">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-[var(--text-secondary)]">{{ $commission->created_at->diffForHumans() }}</p>
                        </div>
                        <span class="text-sm font-bold text-green-500">+${{ number_format($commission->amount, 2) }}</span>
                    </div>
                @empty
                    <p class="text-center text-[var(--text-secondary)] py-4">Aucune commission</p>
                @endforelse
            </div>
            <div class="mt-3 text-right">
                <a href="{{ route('admin.commissions') }}" class="text-sm text-primary-500 hover:text-primary-600 font-semibold transition">
                    Voir toutes →
                </a>
            </div>
        </div>
    ...
    <!-- Actions rapides -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 animate-fadeInUp delay-9">
        <a href="{{ route('admin.users.create') }}" class="card-stats text-center quick-action">
            <div class="stat-icon stat-icon-primary mx-auto mb-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
            </div>
            <p class="text-sm font-semibold text-[var(--text-primary)]">Ajouter un utilisateur</p>
        </a>
        <a href="{{ route('admin.packages.create') }}" class="card-stats text-center quick-action">
            <div class="stat-icon stat-icon-success mx-auto mb-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7l8 4"/>
                </svg>
            </div>
            <p class="text-sm font-semibold text-[var(--text-primary)]">Créer un package</p>
        </a>
        <a href="{{ route('admin.products.create') }}" class="card-stats text-center quick-action">
            <div class="stat-icon stat-icon-info mx-auto mb-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                </svg>
            </div>
            <p class="text-sm font-semibold text-[var(--text-primary)]">Ajouter un produit</p>
        </a>
        <a href="{{ route('admin.commissions') }}" class="card-stats text-center quick-action">
            <div class="stat-icon stat-icon-purple mx-auto mb-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
            </div>
            <p class="text-sm font-semibold text-[var(--text-primary)]">Voir commissions</p>
        </a>
    </div>
</div>
@endsection