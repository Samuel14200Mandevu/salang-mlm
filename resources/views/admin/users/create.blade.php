...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="animate-fadeInUp">
        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-[var(--text-primary)]">Ajouter un utilisateur</h1>
        <p class="text-sm sm:text-base text-[var(--text-secondary)] mt-0.5 sm:mt-1">Creez un nouveau compte utilisateur</p>
    </div>
        ...
        <form action="{{ route('admin.users.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="input text-sm sm:text-base @error('email') input-error @enderror" required>
                    @error('email') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Mot de passe *</label>
                    <input type="password" name="password" class="input text-sm sm:text-base @error('password') input-error @enderror" required>
                    @error('password') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Confirmer *</label>
                    <input type="password" name="password_confirmation" class="input text-sm sm:text-base" required>
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Telephone</label>
                    <input type="tel" name="phone" value="{{ old('phone') }}" class="input text-sm sm:text-base">
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Pays</label>
                    <input type="text" name="country" value="{{ old('country') }}" class="input text-sm sm:text-base">
                </div>
                ...
                <div>
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Parrain (Sponsor)</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2" class="input text-sm sm:text-base">{{ old('address') }}</textarea>
                </div>
            </div>
