...
<div class="space-y-6">
    ...
    <div class="animate-fadeInUp">
        <h1 class="text-2xl md:text-3xl font-bold text-[var(--text-primary)]">✏️ Modifier {{ $product->name }}</h1>
        <p class="text-[var(--text-secondary)] mt-1">ID: #{{ $product->id }}</p>
    </div>
    ...
    <div class="card animate-fadeInUp delay-1 max-w-2xl">
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4 p-4 bg-[var(--bg-secondary)] rounded-lg">
                <p class="text-sm text-[var(--text-secondary)] mb-2">Image actuelle</p>
                ...
                @if($product->image)
                    <p class="text-xs text-[var(--text-secondary)] mt-2">{{ $product->image }}</p>
                @endif
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">Nouvelle image</label>
                    <div class="image-preview-empty" id="imagePreview" onclick="document.getElementById('imageInput').click()">
                        <span class="text-[var(--text-tertiary)] text-2xl">📸 Changer</span>
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" class="input @error('name') input-error @enderror" required>
                    @error('name') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">Slug *</label>
                    <input type="text" name="slug" value="{{ old('slug', $product->slug) }}" class="input @error('slug') input-error @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="price" step="0.01" value="{{ old('price', $product->price) }}" class="input @error('price') input-error @enderror" required>
                    @error('price') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">Coût d'achat ($)</label>
                    <input type="number" name="cost" step="0.01" value="{{ old('cost', $product->cost) }}" class="input">
                </div>

                <div>
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">Stock *</label>
                    <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" class="input @error('stock') input-error @enderror" required>
