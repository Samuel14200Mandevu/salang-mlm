...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="animate-fadeInUp">
        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-[var(--text-primary)]">Ajouter un rang</h1>
        <p class="text-sm sm:text-base text-[var(--text-secondary)] mt-0.5 sm:mt-1">Creez un nouveau grade</p>
    </div>
        ...
        <form action="{{ route('admin.ranks.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div>
                    ...
                    <p class="text-[10px] sm:text-xs text-[var(--text-secondary)] mt-1">URL unique (ex: manager)</p>
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">PV minimum *</label>
                    <input type="number" name="min_pv" value="{{ old('min_pv', 0) }}" class="input text-sm sm:text-base @error('min_pv') input-error @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="min_bv" value="{{ old('min_bv', 0) }}" class="input text-sm sm:text-base">
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Bonus (%) *</label>
                    <input type="number" name="bonus_percentage" step="0.01" value="{{ old('bonus_percentage', 0) }}" class="input text-sm sm:text-base @error('bonus_percentage') input-error @enderror" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="2" class="input text-sm sm:text-base">{{ old('description') }}</textarea>
                </div>
            </div>
