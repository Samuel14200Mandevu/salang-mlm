...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 animate-fadeInUp">
        <div>
            <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-[var(--text-primary)]">Historique des promotions</h1>
            <p class="text-sm sm:text-base text-[var(--text-secondary)] mt-0.5 sm:mt-1">Suivez toutes les promotions de grades</p>
        </div>
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="text-xs sm:text-sm hidden sm:table-cell">Ancien grade</th>
                        <th class="text-xs sm:text-sm">Nouveau grade</th>
                    ...
                    @forelse($history ?? [] as $item)
                        <tr data-user="{{ $item->user_id }}">
                            ...
                            <td class="hidden sm:table-cell">
                                <span class="badge badge-neutral text-[10px] sm:text-xs">{{ $item->old_rank_name ?? 'Debut' }}</span>
                            </td>
                            <td>
                                <span class="badge badge-success text-[10px] sm:text-xs">{{ $item->new_rank_name }}</span>
                            </td>
                            <td class="hidden md:table-cell text-sm sm:text-base">{{ number_format($item->pv_at_time) }}</td>
                            <td class="hidden lg:table-cell text-[var(--text-secondary)] text-xs sm:text-sm">{{ $item->notes ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
