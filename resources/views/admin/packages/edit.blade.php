...
<div class="space-y-6">
    ...
    <div class="animate-fadeInUp">
        <h1 class="text-2xl md:text-3xl font-bold text-[var(--text-primary)]">✏️ Modifier {{ $package->name }}</h1>
        <p class="text-[var(--text-secondary)] mt-1">ID: #{{ $package->id }}</p>
    </div>
        ...
        <form action="{{ route('admin.packages.update', $package) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $package->name) }}" class="input @error('name') input-error @enderror" required>
                    @error('name') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">Slug *</label>
                    <input type="text" name="slug" value="{{ old('slug', $package->slug) }}" class="input @error('slug') input-error @enderror" required>
                    @error('slug') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">Prix ($) *</label>
                    <input type="number" name="price" step="0.01" value="{{ old('price', $package->price) }}" class="input @error('price') input-error @enderror" required>
                    @error('price') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">PV *</label>
                    <input type="number" name="pv_value" value="{{ old('pv_value', $package->pv_value) }}" class="input @error('pv_value') input-error @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="bv_value" value="{{ old('bv_value', $package->bv_value) }}" class="input">
                </div>

                <div>
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-1">Commission (%) *</label>
                    <input type="number" name="commission_rate" step="0.01" value="{{ old('commission_rate', $package->commission_rate) }}" class="input @error('commission_rate') input-error @enderror" required>
