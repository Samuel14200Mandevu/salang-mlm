...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="animate-fadeInUp">
        ...
        <p class="text-sm sm:text-base text-[var(--text-secondary)] mt-0.5 sm:mt-1">Ajoutez des fonds à votre portefeuille</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div class="card animate-fadeInLeft delay-2 p-3 sm:p-4 md:p-6">
            <h3 class="font-semibold text-[var(--text-primary)] text-sm sm:text-base mb-3 sm:mb-4">Formulaire de dépôt</h3>
            
            <form action="{{ route('wallet.deposit.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-3 sm:mb-4">
                    ...
                    @enderror
                    <p class="text-[10px] sm:text-xs text-[var(--text-secondary)] mt-1">Montant minimum: $1.00</p>
                </div>

                <div class="mb-3 sm:mb-4">
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-2">Méthode de dépôt</label>
                    <div class="grid grid-cols-3 gap-2">
                        <div class="method-card selected" data-method="crypto" onclick="selectMethod(this)">
                    ...
                    <select name="payment_method" id="methodSelect" class="hidden" required>
                ...
                <div id="crypto_fields" class="mb-3 sm:mb-4">
                    ...
                    <input type="text" name="crypto_address" class="input text-sm sm:text-base" placeholder="0x... ou adresse TRC20">
                ...
                <div id="mobile_fields" class="mb-3 sm:mb-4 hidden">
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Numéro de téléphone</label>
                ...
                <div id="bank_fields" class="mb-3 sm:mb-4 hidden">
                    <label class="block text-xs sm:text-sm font-medium text-[var(--text-secondary)] mb-1">Coordonnées bancaires</label>
                ...
                <button type="submit" class="btn btn-primary w-full text-sm sm:text-base py-2 sm:py-2.5">
                    ...
                    Déposer $<span id="displayAmount">0.00</span>
                </button>
            </form>
        </div>
        ...
        <div class="card animate-fadeInRight delay-3 p-3 sm:p-4 md:p-6">
            <div class="flex items-center justify-between mb-3 sm:mb-4">
                <h3 class="font-semibold text-[var(--text-primary)] text-sm sm:text-base">Historique des dépôts</h3>
                <span class="badge badge-neutral text-[10px] sm:text-xs">{{ $deposits->count() ?? 0 }} dépôts</span>
            </div>
                ...
                @forelse($deposits ?? [] as $deposit)
                    <div class="flex flex-wrap items-center justify-between p-2 sm:p-3 bg-[var(--bg-secondary)] rounded-lg hover:bg-[var(--bg-hover)] transition-colors gap-1 sm:gap-2">
                        <div class="min-w-0 flex-1">
                            <p class="font-semibold text-[var(--text-primary)] text-sm sm:text-base">+${{ number_format($deposit->amount, 2) }}</p>
                            <p class="text-[10px] sm:text-xs text-[var(--text-secondary)]">
                                {{ ucfirst($deposit->type) }} • {{ $deposit->created_at->format('d/m/Y H:i') }}
                            </p>
                        </div>
                        <div>
                            <span class="badge {{ $deposit->status == 'completed' ? 'badge-success' : ($deposit->status == 'pending' ? 'badge-warning' : 'badge-danger') }} text-[10px] sm:text-xs">
                                {{ ucfirst($deposit->status) }}
                            </span>
                        </div>
                    </div>
                @empty
                    <p class="text-center text-[var(--text-secondary)] py-6 sm:py-8 text-sm">Aucun dépôt effectué</p>
                @endforelse
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (amountInput) {
        amountInput.addEventListener('input', function() {
            document.getElementById('displayAmount').textContent = this.value || '0.00';
        });
    }
});
</script>
