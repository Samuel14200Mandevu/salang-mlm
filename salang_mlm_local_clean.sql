...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 */ /*!50003 TRIGGER `after_order_item_insert` AFTER INSERT ON `order_items` FOR EACH ROW BEGIN
...
--
-- Dumping events for database 'salang_mlm'
--

--
-- Dumping routines for database 'salang_mlm'
--
/*!50003 DROP PROCEDURE IF EXISTS `update_all_team_pv` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
DELIMITER ;;
CREATE PROCEDURE `update_all_team_pv`()
BEGIN
    ...
    -- Table temporaire pour les résultats
    DROP TEMPORARY TABLE IF EXISTS temp_team_pv_calc;
    CREATE TEMPORARY TABLE temp_team_pv_calc (
    ...
    -- Insérer tous les utilisateurs actifs
    INSERT INTO temp_team_pv_calc (user_id)
    SELECT id FROM users WHERE is_active = 1;

    -- Pour chaque utilisateur, calculer les PV d'équipe
    OPEN cur;
    ...
    read_loop: LOOP
        ...
        -- Calcul récursif avec CTE (MySQL 8+)
        WITH RECURSIVE team_tree AS (
            -- Niveau 1: Les enfants directs
            SELECT 
                id,
                pv_balance,
                bv_balance,
                1 AS depth
            FROM users
            WHERE parrain_id = user_id AND is_active = 1
            
            UNION ALL
            
            -- Niveau récursif: Les descendants
            SELECT 
                u.id,
                u.pv_balance,
                u.bv_balance,
                tt.depth + 1
            FROM users u
            INNER JOIN team_tree tt ON u.parrain_id = tt.id
            WHERE u.is_active = 1
                AND tt.depth < 9 -- Limite de sécurité
        )
        SELECT 
            COALESCE(SUM(pv_balance), 0),
            COALESCE(SUM(bv_balance), 0),
            COUNT(*)
        INTO total_pv, total_bv, user_count
        FROM team_tree;

        -- Mettre à jour les résultats
        UPDATE temp_team_pv_calc 
        SET 
            team_pv = total_pv,
            team_bv = total_bv,
    ...
    UPDATE users u
    INNER JOIN temp_team_pv_calc t ON t.user_id = u.id
    SET 
        u.team_pv = t.team_pv,
        u.team_bv = t.team_bv,
        u.total_team = t.total_members
    WHERE u.is_active = 1;

    DROP TEMPORARY TABLE temp_team_pv_calc;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2026-07-15 10:03:00
